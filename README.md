...
| [`port-to-scikit-build-core`](skills/port-to-scikit-build-core/) | Experimental | Migrate a package's build from bespoke `setup.py` to **scikit-build-core + CMake** (PEP 517/621, standards-based wheels, editable installs). |
...
| [`port-to-free-threaded-python`](skills/port-to-free-threaded-python/) | Experimental | Make a package work on the **free-threaded (no-GIL)** CPython build (PEP 703; `cp313t`/`cp314t`) and **declare support** so import doesn't re-enable the GIL — thread-safety pass, `Py_mod_gil` declaration, borrowed→strong reference fixes, `cp3Xt` wheels, TSan / `pytest-run-parallel`. Detects all unsafe shared state; fixes simple cases, reports complex ones. |

...
| [`port-to-python-limited-api`](skills/port-to-python-limited-api/) | Experimental | Port a hand-written C/C++ extension to the **Limited API / stable ABI** — one **abi3** wheel across Python versions instead of one per version (static types → heap types, module state, multi-phase init), optionally targeting 3.15 **abi3t** for free-threaded builds. |
| [`port-to-free-threaded-python`](skills/port-to-free-threaded-python/) | Experimental | Make a package work on the **free-threaded (no-GIL)** CPython build ([PEP 703](https://peps.python.org/pep-0703/); `cp313t`/`cp314t`) and **declare support** so import doesn't re-enable the GIL — thread-safety pass, `Py_mod_gil` declaration, borrowed→strong reference fixes, `cp3Xt` wheels, TSan / `pytest-run-parallel`. Detects all unsafe shared state; fixes simple cases, reports complex ones. |
